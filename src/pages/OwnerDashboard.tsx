...
const OwnerDashboard: React.FC = () => {
  ...
  const fetchProperties = async () => {
    ...
    try {
      setLoading(true);
      const res = await apiClient.get(`/owners/${auth.user.id}/properties`);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await apiClient.delete(`/owners/${auth?.user?.id}/properties/${id}`);
    ...
    <div>
      ...
      <div className="p-6">
        <div className="flex justify-between mb-4">
          <Button className="bg-blue-600 text-white hover:bg-blue-700" onClick={() => { setEditPropertyId(null); setShowForm(!showForm); }}>

// src/controllers/admin/ownersController.ts
...
export async function getOwner(req: Request, res: Response) {
  try {
    ...
    if (!o) return res.status(404).json({ message: "Owner not found" });
    res.json(o);
  } catch (err) {
    console.error(err);
...
export async function createOwner(req: Request, res: Response) {
  try {
    ...
    const user = await User.create({
      ...
      status: "ACTIVE",
      isVerified: false,
      notificationPreferences: { email: true, whatsapp: false, sms: false },
...
export async function updateOwner(req: Request, res: Response) {
  try {
    ...
    if (req.body.name || req.body.email) {
      ...
      if (user) await user.update({ name: req.body.name || user.name, email: req.body.email || user.email });
    }
